...
function MemberFilters({ memberships, tiers }: MemberFiltersProps) {
		...
		<FilterBar
			filters={[
				...
				},
				{
					...
					onValueChange: (value) => updateParam("paymentStatus", value),
				},
			]}
			trailing={
				<>
					<div className="grid gap-2">
						...
						<Input
							...
							onChange={(event) =>
								updateParam("joinedFrom", event.target.value)
							}
						/>
					</div>
					<div className="grid gap-2">
						...
						<Input
							...
							onChange={(event) => updateParam("joinedTo", event.target.value)}
						/>
					</div>
					<SearchInput
